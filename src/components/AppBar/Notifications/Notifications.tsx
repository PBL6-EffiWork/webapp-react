...
function Notifications() {
  ...
  const updateBoardInvitation = (status: string, invitationId: string) => {
    ...
    dispatch(updateBoardInvitationAPI({ status, invitationId }))
      .then(res => {
        // console.log(res)
        const payload = res.payload as { boardInvitation: { status: string, boardId: string } };
        if (payload.boardInvitation.status === BOARD_INVITATION_STATUS.ACCEPTED) {
          const payload = res.payload as { boardInvitation: { status: string, boardId: string } };
          navigate(`/boards/${payload.boardInvitation.boardId}`)
        }
      })
  }
