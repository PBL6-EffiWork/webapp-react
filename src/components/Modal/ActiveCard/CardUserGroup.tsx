...
interface CardUserGroupProps {
  ...
  onUpdateCardMembers: (memberInfo: { userId: string; action: string }) => void;
}

function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }: CardUserGroupProps) {
  /**
   ...
   */
  const FE_CardMembers = board?.FE_allUsers
    ? cardMemberIds.map(id => board.FE_allUsers?.find((u: { _id: string }) => u._id === id) ?? [])
    : [];
  // console.log('FE_CardMembers: ', FE_CardMembers)

  const handleUpdateCardMembers = (user: { _id: string; displayName?: string; avatar?: string }) => {
    // console.log(user)
    ...
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      >
        <Box sx={{ p: 2, maxWidth: '260px', display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
          {board?.FE_allUsers?.map((user: { _id: string; displayName?: string; avatar?: string }, index: number) =>
            <Tooltip title={user?.displayName} key={index}>
