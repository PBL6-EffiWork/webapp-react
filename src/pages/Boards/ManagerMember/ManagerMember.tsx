...
import { ChevronDown, MoreHorizontal, ChevronLeft, ChevronRight, Bell, UserMinus } from 'lucide-react'
import { useDispatch, useSelector } from "react-redux"
import { loadUpcomingTaskThunk, removeMemberFromBoardThunk, selectBoardMembersId, selectMembersUpcomingTask } from "../../../redux/board/boardSlice"
...
export default function ManagerMembersTable({ boardId }: ManagerMembersTableProps) {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("upcomingTask", {
        header: t('upcomingTask'),
        cell: (info) => {
          const memberId = info.row.original._id
          const upcomingTask = upcomingTasks.filter(Boolean).find((task) => task.memberIds.includes(memberId))
          return upcomingTask?.title || "N/A";
        },
      }),
      columnHelper.accessor("dueDateTask", {
        header: t('dueDateTask'),
        cell: (info) => {
          const memberId = info.row.original._id
          const upcomingTask = upcomingTasks.filter(Boolean).find((task) => task.memberIds.includes(memberId))
          return upcomingTask?.dueDate ? new Date(upcomingTask.dueDate).toLocaleDateString() : "N/A"
        },
      }),
